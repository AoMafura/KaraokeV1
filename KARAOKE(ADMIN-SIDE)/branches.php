<?php include ('branch-top.php') ?>
<?php include ('left.php') ?>

<div class = "col-md-9 right">
    <h3 class = "mt-4 mb-3">Branches</h3>
    <h6>Search Branch:</h6>
    <input type="text" class="form-control form-control-sm mr-2" style="max-width: 20rem; float: left;" placeholder = "Enter Branch ID...">
    <button class = "btn btn-primary btn-sm" style = "max-width: 4rem;">Filter</button>
    <button class = "btn btn-primary btn-sm" style="float: right;">Create New</button>
    <center>
    <table class="table text-dark mt-4">
        <thead>
            <tr>
                <th scope="col">Lorem Ipsum</th>
                <th scope="col">Lorem Ipsum</th>
                <th scope="col">Lorem Ipsum</th>
                <th scope="col"></th>
                <th scope="col"></th>
            </tr>
        </thead>
        <tbody>
            <tr class="table-light">
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Update</a></td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Delete</a></td>
            </tr>
            <tr class="table-light">
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Update</a></td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Delete</a></td>
            </tr>
            <tr class="table-light">
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Update</a></td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Delete</a></td>
            </tr>
            <tr class="table-light">
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Update</a></td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Delete</a></td>
            </tr>
            <tr class="table-light">
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4">Lorem Ipsum</td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Update</a></td>
                <td class = "p-4"><a class = "btn btn-primary btn-sm text-white" style = "max-width: 5rem;">Delete</a></td>
            </tr>
        </tbody>
    </table> 
    </center>
</div>

<?php include ('footer.php') ?>